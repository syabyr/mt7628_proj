...
static inline uint32_t mt7620a_pcm_read(const struct mt7620a_pcm *pcm,
	unsigned int reg)
{
	return readl(pcm->base + reg);
}

static inline void mt7620a_pcm_write(const struct mt7620a_pcm *pcm,
	unsigned int reg, uint32_t value)
{
	writel(value, pcm->base + reg);
}

static int mt7620a_pcm_startup(struct snd_pcm_substream *substream,
